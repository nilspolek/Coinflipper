  ...
  template: `
    <div class="row justify-content-center m-auto">
      @for (card of cards; track card.name) {
        <div class="card col-sm-3 col-12 m-2 p-3 shadow">
          <h1 class="card-title pb-3">{{ card.name }}</h1>
          <img class="rounded mb-3 shadow" alt="lotto" [src]="card.img" />
          <a class="btn btn-primary shadow" [routerLink]="card.link">Draw numbers</a>
        </div>
      }
    </div>
  `,
