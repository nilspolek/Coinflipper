...
export class CardComponent {
  getACard = async (): Promise<any> => {
    ...
    try {
      ...
      const suitIndex = Math.floor(
        (responseData.result.random.data[0] - 1) / 13,
      );
  ...
  data = {
    ...
    mainText:
      'Discover endless possibilities with our virtual deck! Click to draw a random card and let destiny unfold. Perfect for games, decision-making, or sparking creativity',
    result: '',
    resultBannerFirst: "Congratulations! You've drawn a card with the value ",
    resultBannerLast:
      " of Diamonds! 🃏 Whether you're playing a game of cards or just adding some excitement, our card drawer is here to bring a touch of randomness to your day. Feel the anticipation with every draw as you reveal the card from the deck, showing its value. Let the games begin!",
